...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    const diveSeg = SECTIONS[near]._seg;
    const sectionProgress = diveSeg ? clamp((y - diveSeg.start) / (diveSeg.end - diveSeg.start), 0, 1) : 0;
    const act = SECTIONS[near].act || 'customer';
    const journeyEnd = totalW * vh;
    const fadeStart = Math.max(0, journeyEnd - vh * 0.4);
    const journeyOpacity = y >= fadeStart ? clamp(1 - (y - fadeStart) / (vh * 0.4), 0, 1) : 1;
    container.style.setProperty('--sw-journey-opacity', journeyOpacity);
    if (journeyOpacity < 0.02) container.classList.add('is-journey-done');
    else container.classList.remove('is-journey-done');
    document.documentElement.style.setProperty('--sw-journey-opacity', journeyOpacity);
    if (near !== activeIndex) {
      activeIndex = near;
      dots.forEach((d, k) => d.classList.toggle('is-active', k === near));
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => n.classList.toggle('is-active', k === near));
      container.style.setProperty('--sw-accent', SECTIONS[near].accent || '');
      document.documentElement.dataset.scrollAct = act;
      if (typeof config.onActiveIndexChange === 'function') config.onActiveIndexChange(near);
    }
    if (typeof config.onScrollStateChange === 'function') {
      config.onScrollStateChange({ activeIndex: near, sectionProgress, act, journeyOpacity });
    }
    window.dispatchEvent(new CustomEvent('sw-scroll-state', { detail: { activeIndex: near, sectionProgress, act, journeyOpacity } }));
...
function injectCSS() {
  ...
  const css = `
  .sw-root{--sw-bg:#f5f8f6;--sw-ink:#14201a;--sw-ink-soft:#5f6d66;--sw-accent:#40916c;--sw-journey-opacity:1;
    --sw-font-display:'Vazirmatn Variable',Vazirmatn,ui-sans-serif,system-ui,sans-serif;
    --sw-font-body:'Vazirmatn Variable',Vazirmatn,ui-sans-serif,system-ui,sans-serif;
    color:var(--sw-ink);font-family:var(--sw-font-body);direction:rtl;}
  html,body{margin:0;background:var(--sw-bg,#F5EDE0);overflow-x:hidden;transition:background-color .75s ease;}
  .sw-sky{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none;background:var(--sw-bg);opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  ...
  .sw-scrollbar{position:fixed;top:0;left:0;right:0;height:3px;z-index:60;background:color-mix(in srgb,var(--sw-accent) 14%,transparent);opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  .sw-scrollbar span{display:block;height:100%;width:100%;transform-origin:0 50%;transform:scaleX(0);background:var(--sw-accent);}
  .sw-topbar{position:fixed;top:0;left:0;right:0;z-index:50;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:clamp(14px,2.4vw,26px) clamp(18px,5vw,64px);opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  ...
  .sw-stage{position:fixed;inset:0;z-index:10;pointer-events:none;opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  .sw-scene{position:absolute;inset:0;opacity:0;overflow:hidden;will-change:opacity;}
  .sw-scene__video,.sw-scene__still{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 42%;}
  .sw-scene__still{will-change:transform;} .sw-scene.has-clip .sw-scene__still{opacity:0;} .sw-scene__video{z-index:1;}
  .sw-copylayer{position:fixed;inset:0;z-index:20;pointer-events:none;opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  .sw-copylayer::before{content:"";position:absolute;inset:0;left:auto;right:0;width:min(52vw,680px);background:linear-gradient(270deg,var(--sw-bg) 0%,color-mix(in srgb,var(--sw-bg) 88%,transparent) 40%,transparent 100%);}
  .sw-copy{position:absolute;right:clamp(18px,5vw,64px);left:auto;top:50%;transform:translateY(-50%);width:min(42vw,460px);opacity:0;will-change:opacity,transform;text-align:right;}
  .sw-copy__num{font-family:ui-monospace,Menlo,monospace;font-size:.74rem;letter-spacing:.04em;color:var(--sw-ink-soft);}
  .sw-copy__eyebrow{display:block;margin-top:18px;font-family:var(--sw-font-display);font-weight:700;font-size:.92rem;letter-spacing:0;color:var(--sw-accent);}
  ...
  .sw-route{position:fixed;left:clamp(14px,2.4vw,30px);right:auto;top:50%;z-index:40;transform:translateY(-50%);display:flex;flex-direction:column;gap:22px;padding:18px 10px;opacity:var(--sw-journey-opacity,1);transition:opacity .4s ease;}
  ...
  .sw-route__label{position:absolute;left:24px;right:auto;top:50%;transform:translateY(-50%) translateX(-6px);white-space:nowrap;font-size:.78rem;font-weight:600;color:var(--sw-ink);background:color-mix(in srgb,#fff 85%,transparent);backdrop-filter:blur(6px);padding:5px 11px;border-radius:999px;opacity:0;pointer-events:none;transition:opacity .25s,transform .25s;border:1px solid color-mix(in srgb,var(--sw-accent) 14%,transparent);}
  .sw-route__dot:hover .sw-route__label,.sw-route__dot.is-active .sw-route__label{opacity:1;transform:translateY(-50%) translateX(0);}
  .sw-hint{position:fixed;left:50%;bottom:26px;z-index:30;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:10px;font-size:.76rem;letter-spacing:.14em;text-transform:uppercase;color:var(--sw-ink-soft);transition:opacity .3s;opacity:var(--sw-journey-opacity,1);}
  .sw-root.is-journey-done .sw-sky,.sw-root.is-journey-done .sw-stage,.sw-root.is-journey-done .sw-copylayer,.sw-root.is-journey-done .sw-route,.sw-root.is-journey-done .sw-hint,.sw-root.is-journey-done .sw-topbar,.sw-root.is-journey-done .sw-scrollbar{visibility:hidden;pointer-events:none;}
  .sw-root.is-journey-done ~ .live-mockup-layer{visibility:hidden;pointer-events:none;opacity:0;}
  ...
  @media (max-width:860px){
    ...
    .sw-copy{right:clamp(18px,5vw,64px);left:clamp(18px,5vw,64px);top:auto;bottom:clamp(64px,14vh,120px);transform:none;width:auto;max-width:560px;}
    ...
    .sw-route{gap:16px;left:6px;right:auto;} .sw-route__label{display:none;}
  }
